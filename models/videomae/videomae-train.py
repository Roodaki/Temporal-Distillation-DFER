...
from transformers import VivitImageProcessor, VideoMAEForVideoClassification
...
MODEL_CHECKPOINT = "MCG-NJU/videomae-base-finetuned-kinetics"  # <--- CHANGED
...
SAVE_DIR = "/home/hr/Roodaki/VFER/Emognition Codebase/models/videomae/checkpoints/videomae-base-finetuned-kinetics"  # <--- CHANGED
BEST_MODEL_SAVE_PATH = os.path.join(SAVE_DIR, "best_videomae.pth")  # <--- CHANGED
LAST_CHECKPOINT_PATH = os.path.join(SAVE_DIR, "last_videomae.pth")  # <--- CHANGED
...
NUM_WORKERS = max(0, os.cpu_count() - 32)
...
def main():
    ...
    # Note: VideoMAE can use the same processor as Vivit
    processor = VivitImageProcessor.from_pretrained(MODEL_CHECKPOINT)
    model = VideoMAEForVideoClassification.from_pretrained(  # <--- CHANGED
    ...
    for param in model.videomae.parameters():  # <--- CHANGED
        param.requires_grad = False
    print("Froze `model.videomae` backbone.")  # <--- CHANGED
