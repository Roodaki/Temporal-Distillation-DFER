...
NUM_RANDOM_SEGMENTS_TO_SAMPLE = 1
...
# =========================================================================
# Emotion-guided worker
# =========================================================================


def process_single_video_emotion_worker(task_args_tuple):
    (
...
def run_emotion_pass(args, trim_window_size):
    source_videos_root = pathlib.Path(args.source_dir)
    pre_analyzed_root = pathlib.Path(args.logs)
    trimmed_output_root = pathlib.Path(args.output_dir) / "emotion"
    ...
    with mp.Pool(processes=num_processes_to_use) as pool:
        results = pool.map(process_single_video_emotion_worker, tasks_args_list)

    print("\n" + "=" * 50)
    print("Emotion-guided Pass Summary:")
    ...
    print(
        f"Emotion-guided pass finished in {time.time() - start_time_script:.2f} seconds."
    )
...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Temporal distillation: emotion-guided and/or random trimming."
    )
    parser.add_argument(
        "--mode",
        type=str,
        choices=["emotion", "random", "both"],
        default="both",
        help="Which trimming strategy to run.",
    )
    parser.add_argument(
        "--source_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root of per-video emotion CSV logs produced by analyze_vid    eos.py. "
        "Only required for --mode emotion or --mode both.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="Root output directory. Emotion-guided clips go in <output_dir>/emotion, "
...
if __name__ == "__main__":
    ...
    if args.mode in ("emotion", "both"):
        run_emotion_pass(args, TRIM_WINDOW_SIZE)
